...
class Data(Dataset):
    def __init__(self, cfg):
        with open(os.path.join(cfg.datapath , cfg.mode+'.txt'), 'r') as lines:
            self.samples = []
            for line in lines:
                imagepath = os.path.join(cfg.datapath, line.strip())
                maskpath  = os.path.join(cfg.datapath, line.strip())
...
if __name__=='__main__':
    ...
    #cfg  = Config(mode='train', datapath='/home/amaxv1004/Data/LXY/ROI/GLO/tools/PASM_gt')
    data = Data(cfg)
    for i in range(100):
